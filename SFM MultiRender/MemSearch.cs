...
namespace SFM_MultiRender
{
    ...
    public class MemorySearch
    {
        ...
        private void dumpRegions()
        {
            ...
            Console.WriteLine("Max= " + _maxAddress.ToString("X"));
        ...
        public byte[] readMemoryPointerInt(IntPtr lpBaseAddress, int[] offsets)
        {
            foreach (int offset in offsets)
            {
                byte[] buffer = readMemory(lpBaseAddress,8);
                lpBaseAddress = (IntPtr)BitConverter.ToInt32(buffer, 0);
                lpBaseAddress += offset;
            }
            return readMemory(lpBaseAddress,8);
        }
